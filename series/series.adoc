...
Related URL include both HATEOAS hypermedia links and OGC14-055r2 style `offerings`.
...
a| potentialAction +
~$.potentialAction~
| Offering (See <<OGC14-055r2>>) available for this collection.
...
----
{
 "distribution": [
      {
        "@type": "DataDownload",
        "contentURL": "https://coronavirus.jhu.edu/map.html",
...
A `MediaObject` is used to represent hypermedia links.  If the link represents a data download link (`rel="enclosure"`) then also `DataDownload` objects can be used as recommended by <<ESIPFed>>.
...
A list of MediaObject properties is given in <<table_object_action>>.
