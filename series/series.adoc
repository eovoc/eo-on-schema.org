...
Actions can be used to encode an OWC `offering` of a service record with multiple operations (OGC 14-055r2). The below example represents an offering corresponding to an OGC Web Coverage Service.  The service metadata record would use `potentialAction` to relate the service metadata record with one of its offerings.  as the `Action` uses the service, it is encoded as a `UseAction`.
