...
This section defines the encoding of Earth Observation collection metadata.  We use a presentation which is similar to the GeoJSON Encoding Specifications in <<OGC17-003>>, <<OGC17-047>> and <<OGC17-084>>.  The graphical notation is borrowed from https://github.com/ESIPFed/science-on-schema.org/tree/master/assets/diagrams[science-on-schema.org]. The diagrams are prepared with PlantUML.
...
|===
| Property |  schema.org | schema.org Reference | Reference

a| abstract +
~$.properties.abstract~     |  $.description      | https://schema.org/description  |    <<table_object_dataset>>

a| title +
~$.properties.title~        | $.name              | https://schema.org/name |    <<table_object_dataset>>
